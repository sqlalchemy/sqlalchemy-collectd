...
def update_total_checkouts(values_obj, value, hostprog):
    ...
    if hostprog.last_total_checkout_time:
        ...
        #        print("time delta %s  last time %s now %s" % (time_delta, hostprog.last_total_checkout_time, values_obj.time))
        #        print("prev checkouts %s now checkouts %s per interval %s" % (hostprog.total_checkouts, total_checkouts, hostprog.interval_checkouts))
        if time_delta >= values_obj.interval and hostprog.total_checkouts > 0:
            hostprog.checkouts_per_second = (
                hostprog.interval_checkouts
            ) / time_delta
    #           print("so we calculat4ed as %s" % hostprog.checkouts_per_second)
